...
const enTranslations = {
  ...
  "household.error.rls.insert": "Permission Denied: Could not create the household. This is caused by a database security rule. Please go to your Supabase dashboard, find the 'households' table under Authentication > Policies, and ensure you have an active policy FOR INSERT that allows authenticated users to add rows. A standard condition for this is `(auth.uid() = owner_id)`.",
...
const deTranslations = {
  ...
  "household.error.rls.insert": "Zugriff verweigert: Der Haushalt konnte nicht erstellt werden. Dies wird durch eine Datenbanksicherheitsregel verursacht. Bitte gehen Sie zu Ihrem Supabase-Dashboard, suchen Sie die 'households'-Tabelle unter Authentication > Policies und stellen Sie sicher, dass Sie eine aktive Richtlinie FÜR INSERT haben, die es angemeldeten Benutzern erlaubt, Zeilen hinzuzufügen. Eine Standardbedingung hierfür ist `(auth.uid() = owner_id)`.",
