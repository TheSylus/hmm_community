...
const enTranslations = {
  ...
  "household.error.rls": "Permission Denied: A permission error occurred after creating the household. This is most likely due to your Row Level Security (RLS) policies in Supabase. Please ensure you have an ACTIVE policy on the `profiles` table that allows authenticated users to READ (SELECT) and UPDATE their own profile. The condition should be `(auth.uid() = id)`.",
...
const deTranslations = {
  ...
  "household.error.rls": "Zugriff verweigert: Nach dem Erstellen des Haushalts ist ein Berechtigungsfehler aufgetreten. Dies liegt höchstwahrscheinlich an Ihren Row-Level-Security (RLS) Richtlinien in Supabase. Bitte stellen Sie sicher, dass Sie eine AKTIVE Richtlinie für die `profiles`-Tabelle haben, die es authentifizierten Benutzern erlaubt, ihr eigenes Profil zu LESEN (SELECT) und zu AKTUALISIEREN (UPDATE). Die Bedingung sollte `(auth.uid() = id)` lauten.",
