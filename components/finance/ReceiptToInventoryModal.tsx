...
import { XMarkIcon, CheckCircleIcon, PlusCircleIcon, CategoryOtherIcon, CategoryProduceIcon, CategoryBakeryIcon, CategoryMeatIcon, CategoryDairyIcon, CategoryPantryIcon, CategoryFrozenIcon, CategorySnacksIcon, CategoryBeveragesIcon, CategoryHouseholdIcon, CategoryPersonalCareIcon, CategoryRestaurantIcon } from '../Icons';
...
export const ReceiptToInventoryModal: React.FC<ReceiptToInventoryModalProps> = ({ receipt, items, onConfirm, onClose }) => {
    ...
    // Initial Selection Logic: Select Food items, deselect likely Non-Food/Household
    useEffect(() => {
        const initialSelection = new Set<number>();
        items.forEach((item, index) => {
            // Smart Filter: Exclude household/personal_care by default as they aren't "Food Memory" usually
            if (item.category !== 'household' && item.category !== 'personal_care') {
    ...
    const handleImport = async () => {
        ...
        selectedIndices.forEach(index => {
            ...
            foodItemsToCreate.push({
                ...
                isFamilyFavorite: false, // Default private
                price: rItem.price // Capture price history
            });
        });
                    ...
                    {items.map((item, idx) => {
                        ...
                        return (
                            <div 
                                ...
                                className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${isSelected ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 opacity-60'}`}
                            >
                                <div className={`w-5 h-5 rounded-full border flex items-center justify-center flex-shrink-0 transition-colors ${isSelected ? 'bg-indigo-600 border-indigo-600' : 'border-gray-400 bg-transparent'}`}>
                                    {isSelected && <CheckCircleIcon className="w-4 h-4 text-white" />}
                                </div>
                                
                                <div className="flex-1 min-w-0">
                                    <p className={`text-sm font-bold truncate ${isSelected ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}>
                                        {item.raw_name}
                                    </p>
