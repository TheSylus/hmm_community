...
const FoodItemCardContent: React.FC<FoodItemCardProps> = ({ item, onDelete, onEdit, onViewDetails, onAddToShoppingList, onToggleFamilyStatus, isPreview = false, isInShoppingList = false }) => {
  ...
  // Quality Update: Reduced height (h-24 sm:h-28) and padding (p-2) for compactness
  return (
    <div 
        className={`group bg-white dark:bg-gray-800 rounded-xl shadow-sm border overflow-hidden transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 relative flex flex-row h-24 sm:h-28 ${isClickable ? 'cursor-pointer' : ''} ${borderClass} ${ringClass}`}
                    ...
                    {!isPreview && (
                        <div className="flex items-center gap-0.5 -mr-1 -mt-1 shrink-0">
                            ...
                            <button
                                ...
                                className="p-1 rounded-full text-gray-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30 dark:hover:text-red-400 transition-colors"
                            >
